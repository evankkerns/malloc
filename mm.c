...
team_t team = {
    ...
    "evan.kerns@colorado.edu",
    /* Second member's full name (leave blank if none) */
    "Ryan Bohannon",
    /* Second member's email address (leave blank if none) */
    "ryan.bohnannon@colorado.edu"
};

// Mainly original code with some additional code from github
...
static void *extend_heap(size_t size)
{
    // From the book (p. 831) and edited with code from github
 ...
 */
static void insert_node(void *ptr, size_t size) {
    // Not from the book, found on github
    int list = 0;
    void *search_ptr = ptr;
...
static void *coalesce(void *ptr)
{
    // From the book (p. 833) and github
...
{
    // From the book (p. 856/857) and github
    ...
    if (remainder <= DSIZE * 2) {
        PUT(HDRP(bp), PACK(csize, 1)); /* Block header */
        PUT(FTRP(bp), PACK(csize, 1)); /* Block footer */
    }
...
int mm_init(void)
{
    ...
    // From the book (p. 831) and github
...
{
    // From the book (p. 834) and github
...
{
    // From the book (p. 833) and github
    size_t size = GET_SIZE(HDRP(bp)); /* Size of block */
